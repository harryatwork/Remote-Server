						...
						<div class="col-12 col-lg-12">
							<div class="card">
								<div class="card-body">
    								...
    								<?php
    									$sql_parent = "SELECT * FROM items WHERE id = '$f' AND state = 'Live'";
                                    	$result_parent = $conn->query($sql_parent);
                                    	if ($result_parent->num_rows > 0) {   
                                    	while($row_parent = $result_parent->fetch_assoc()) {
                                    		$parent_id = $row_parent["id"];
                                    		
    									    $folders_id = $row_parent["id"];
                                    		$folders_bywhom = $row_parent["bywhom"];
                                    		$folders_name = $row_parent["name"];
                                    		$file_name = $row_parent["file_name"];
                                    		$folders_type = $row_parent["type"];
                                    		$folders_parent = $row_parent["parent"];
                                    		$folders_state = $row_parent["state"];
                                    		$folders_size = $row_parent["size"];
                                    		$folders_timestamp = $row_parent["timestamp"];
                                    		$folders_undeletable = $row_parent["undeletable"];
                                    		$folders_private = $row_parent["private"];
